...
const DEFAULT_BANNER = 'Faux-MySQL-Server/1.0';

const MYSQL_PROTOCOL_VERSION = 10;
const MYSQL_CHARSET_LATIN1 = 0x21;
const MYSQL_ENCODING_ASCII_NUM = 0x0C;
const MYSQL_ENCODING_ASCII_STR = 'ascii';
const MYSQL_ERROR_DEFAUT = 2000;
const MYSQL_STATE_DEFAUT = 'HY000';
...
export default class Server {
  ...
  sendDefinitions(definitions) {
    ...
    for (let definition of definitions) {
      ...
      len = payload.writeUInt8(MYSQL_ENCODING_ASCII_NUM, len);
      len = payload.writeUInt16LE(consts.MYSERVER_CLIENT_CHARSET, len);
  ...
  sendServerHello = () => {
    ...
    let pos = 4;
    pos = payload.writeUInt8(MYSQL_PROTOCOL_VERSION, pos);
    ...
    if (this.serverCharset) {
      ...
      pos = payload.writeUInt8(MYSQL_CHARSET_LATIN1, pos);
    }
  ...
  helloPacketHandler = (packet) => {
    ...
    let username = packet.toString(MYSQL_ENCODING_ASCII_STR, ptr, usernameEnd);
    ptr = usernameEnd + 1;
    ...
    if (databaseEnd >= 0) {
      database = packet.toString(MYSQL_ENCODING_ASCII_STR, ptr, databaseEnd);
    }
    this.onPacket = null;
  ...
  sendError({ message = 'Unknown MySQL error', errno = MYSQL_ERROR_DEFAUT, sqlState = MYSQL_STATE_DEFAUT}) {
    //## Sending Error ...
    console.log(message);
    let data = Buffer.alloc(message.length + 64);
    let len = 4;
